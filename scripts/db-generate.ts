...
import { authDdl } from "../src/auth/ddl";
...
async function main(): Promise<void> {
	...
	// Better Auth tables — emitted alongside the model migrations so the auth
	// schema flows through the SAME pipeline (db:migrate / serve / CF sqlite
	// worker / `wrangler d1 migrations apply`). `betterAuthEnabled()` is a Bun
	// macro: with `BETTER_AUTH_ENABLED=false` the file is skipped, keeping a
	// no-auth build's drizzle/ output free of auth tables.
	if (betterAuthEnabled() && dialect === "sqlite") {
		// The migration filename is `<ts>_sqlite_create.sql` — reuse its
		// timestamp so both files sort adjacently in drizzle/.
		const ts = file.slice(0, -"_sqlite_create.sql".length);
		const authFile = `${ts}_auth_sqlite_create.sql`;
		const authOut = resolve(OUT_DIR, authFile);
		writeFileSync(
			authOut,
			`-- Generated by db-generate — Better Auth tables (from src/auth/schema.ts)\n-- Do not edit by hand: run \`db:generate\` to regenerate.\n\n${authDdl}`,
		);
		console.log(`Generated auth migration for "sqlite" (Better Auth enabled):`);
		console.log(`  ${authOut}`);
	} else if (dialect === "sqlite") {
		console.log(
			"db-generate: BETTER_AUTH_ENABLED=false — skipped the Better Auth migration.",
		);
	}
}
