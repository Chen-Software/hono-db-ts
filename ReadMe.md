...
CodeForge is a **Forgejo-style Git forge** built with
...
| `GET /api/repositories?limit=&cursor=` | cursor-paginated index |
| `GET /api/repositories/:id` | 404 if missing |
| `GET /api/repositories/aggregate` | `Aggregable` grouping (e.g. repos per owner) |
| `GET /api/users/:id` | 404 if missing |
| `GET /api/users/aggregate` | `Aggregable` grouping (e.g. avg age per role) |
