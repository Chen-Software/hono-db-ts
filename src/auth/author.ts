/**
 * author — session→user resolution facade.
 *
 * Historically this module executed the `users`-table SQL itself (upsert on
 * first request, fallback lookup, activity-counter resync). That SQL now lives
 * in the user service (`src/services/users.ts`) so every `sql.unsafe` in the
 ...
 */
export { ensureUser, resolveUser } from '@/services/users'
