 ...
 * `capacities/sql-serialisable.ts`). The model binds zero drizzle column code,
...
const UserSchemaModule: SqlSchemaModule<UserSchema> = {
	...
	assertEquals: typia.createAssertEquals<UserSchema>(),
	validateEquals: typia.createValidateEquals<UserSchema>(),
	assertGuardEquals: typia.createAssertGuardEquals<UserSchema>(),
	assertGuardValidate: typia.createAssertGuard<UserSchema>(),
	// json family
	...
	equals: typia.compare.createEquals<UserSchema>(),
	less: userLess,
 ...
 * then folds the declared capacities (below) onto it. `Triggerable` is
...
const UserModel = defineModel<UserSchema>({
	...
	capacities: [
		...
		Comparable,
		// Referencible: inverse collection accessors (`user.getRepositories()`)
		// are AUTO-DERIVED from the `Reference` tags on the SOURCE models —
		// Repository's `ownerId` targeting `"UserSchema"`.
		// `wireInverseRelations()` (run from `defineModel`) installs them on
		// this prototype and mirrors the tag's `onDelete`, so no manual
		// `relations` entry is needed here. The owner accessor
		// (`repo.getOwner()`) is still derived from the source model's own tag.
		Referencible,
		...
		Randomisable,
		// Queriable: turns `User` into a queryable entity via `User.filter(items,
		// query)`. The WHOLE model is queryable — field semantics are INFERRED from
		// the reflected schema (boolean → eq, number → eq, date → range, array →
		// list, string/uuid → substring). The `fields` override below exposes
		// `email` as the alias `?mail=` in addition to `?email=`.
		{
			capacity: Queriable,
		...
		// recorded as `User.<op>` — visible in `/debug/operations` (dev) and as
