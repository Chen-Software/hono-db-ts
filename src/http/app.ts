...
export function buildQueryApp(client: SqlQueryExecutor): Hono {
	...
	// Generated aggregate routes — `Aggregable` translates the SAME `?param=`
	// surface into a SQL `SELECT … GROUP BY …`:
	...
	//   GET /threads/aggregate?groupBy=boardId&count=*  → "threads per board"
	//   GET /boards/aggregate?groupBy=moderatorId&count=*  GET /replies/aggregate
	// Registered BEFORE the per-model CRUD below: a literal `/users/aggregate`
	// path must win over the generic `GET /users/:id` by-id route (Hono: first
	// registration wins on identical method+path match).
	(Models.Post.Post as any).serveAggregate(app, client);
	(Models.User.User as any).serveAggregate(app, client);
	(Models.Thread.Thread as any).serveAggregate(app, client);
	(Models.Board.Board as any).serveAggregate(app, client);
	(Models.Reply.Reply as any).serveAggregate(app, client);
