 ...
 * it. Serialisation is REUSED from `JsonSerialisable`/`ProtobufEncodable`
 ...
 *   instance.save(): Promise<this>      — serialise + store.put + emit
 *   static  load(id): Promise<T | undefined>
 *   static  find(filter?): Promise<T[]>  — store.query, or in-memory fallback
 */
function Persistable<TBase extends CapacityComposer>(
	Base: TBase,
	_mod?: any,
	...
	return class extends (Base as any) {
		/** Serialise + write to the store, firing before/afterPersist. */
