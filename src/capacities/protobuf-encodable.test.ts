...
const pingModule: SchemaModule<Ping> = {
	...
	clone: typia.plain.createClone<Ping>(),
	assertClone: typia.plain.createAssertClone<Ping>(),
	isClone: typia.plain.createIsClone<Ping>(),
	validateClone: typia.plain.createValidateClone<Ping>(),
	is: typia.createIs<Ping>(),
	assert: typia.createAssert<Ping>(),
	assertGuard: typia.createAssertGuard<Ping>(),
	validate: typia.createValidate<Ping>(),
	assertEquals: typia.createAssertEquals<Ping>(),
	validateEquals: typia.createValidateEquals<Ping>(),
	assertGuardEquals: typia.createAssertGuardEquals<Ping>(),
	assertGuardValidate: typia.createAssertGuard<Ping>(),
	stringify: typia.json.createStringify<Ping>(),
	toJSON: typia.json.createAssertStringify<Ping>(),
	isStringify: typia.json.createIsStringify<Ping>(),
	validateStringify: typia.json.createValidateStringify<Ping>(),
	fromJSON: typia.json.createAssertParse<Ping>(),
	isParse: typia.json.createIsParse<Ping>(),
	validateParse: typia.json.createValidateParse<Ping>(),
	...
	isEncode: typia.protobuf.createIsEncode<Ping>(),
	validateEncode: typia.protobuf.createValidateEncode<Ping>(),
	isDecode: typia.protobuf.createIsDecode<Ping>(),
	validateDecode: typia.protobuf.createValidateDecode<Ping>(),
	equals: typia.compare.createEquals<Ping>(),
	less: typia.compare.createLess<Ping>(),
	more: (x: any, y: any) => typia.compare.createLess<Ping>()(y, x),
	random: typia.createRandom<Ping>(),
};
...
describe("ProtobufEncodable — registration (Triggerable gatekeeper)", () => {
	it("registers itself once Triggerable has paved the registry", () => {
		const Model = ProtobufEncodable(Triggerable(makeModel()), pingModule);
		const caps = (Model as unknown as WithCapacities).prototype.capacities;
		expect(caps).toBeDefined();
		expect([...caps!]).toContain("Triggerable");
		expect([...caps!]).toContain("ProtobufEncodable");
	});

	it("does NOT register when Triggerable is absent (guard short-circuits)", () => {
	...
	it("still attaches the codec even without Triggerable", () => {
...
describe("ProtobufEncodable — composes after JsonSerialisable (real wiring)", () => {
	it("registers both capacities and exposes both codecs", () => {
		const Composed = ProtobufEncodable(
			JsonSerialisable(Triggerable(makeModel()), pingModule),
			pingModule,
		);
