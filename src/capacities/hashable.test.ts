...
describe("FORMAT-only vs SEMANTIC correctness — the two layers", () => {
	...
	it("Format-only tag REJECTS a malformed hash", () => {
		expect(validateFormatOnly({ contentHash: "not-a-hash" }).success).toBe(
			false,
		);
	});
...
describe("withContentHash", () => {
	it("round-trips for the default content key", () => {
		...
		expect(addressed.contentHash).toBe(hashContent("hello world"));
	...
	it("overwrites any incoming hash", () => {
		const addressed = withContentHash(
			{ content: "hello", contentHash: PLACEHOLDER },
			"content",
		);
		expect(addressed.contentHash).toBe(hashContent("hello"));
	});
});
...
describe("createAssertHash (construction-time stamping)", () => {
	...
	it("overwrites any caller-supplied hash (caller need not compute it)", () => {
		const addressed = assertBody({ body: "hello", contentHash: PLACEHOLDER });
		expect(addressed.contentHash).toBe(hashContent("hello"));
	});
	...
	it("is generic over the entity shape (carries other fields through)", () => {
		...
		expect(addressed.contentHash).toBe(hashContent("x"));
	});
});
...
describe("updateHash (update-time rehash)", () => {
	...
	it("recomputes the hash when content changes AND bumps the version", () => {
		...
		expect(next.contentHash).toBe(hashContent("edited"));
	...
	it("recomputes the hash idempotently when content is unchanged", () => {
		...
		expect(next.contentHash).toBe(hashContent("original"));
...
describe("createContentAddressing(key)", () => {
	...
	it("assertHash stamps the correct hash from the named key", () => {
		const out = CA.assertHash({ body: "hello", contentHash: "WRONG-INPUT" });
		expect(out.contentHash).toBe(hashContent("hello"));
	});
	...
	it("updateForVersionable is the bound, versioned update factory (key captured)", () => {
		// `updateForVersionable` composes `updateHash` (version bump + rehash),
		// so it requires a `Versionable` entity. The behaviour is covered
		// end-to-end by the Post model tests and the `updateHash` block above.
		expect(typeof CA.updateForVersionable).toBe("function");
	});
});
